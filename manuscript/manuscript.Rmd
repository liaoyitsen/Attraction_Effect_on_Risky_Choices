...
study1 %>%
  ...
  dplyr::mutate(condition = dplyr::recode_factor(condition,
                                                             'risk_averse' = 'Safe condition',
                                                             'risk_seeking' = 'Risky condition')) %>%
  dplyr::mutate(choose_risky = ifelse(raw_choice == "risky", 1, 0),
                choose_safe = ifelse(raw_choice == "safe", 1, 0))%>%
  dplyr::group_by(condition) %>%
  dplyr::summarize(mean = mean(choose_risky),
                   n = dplyr::n(),
                   sd = sd(choose_risky),
                   ci = qnorm(0.975) * sd/sqrt(n),
                   .groups = "drop") %>%
  ...
  ggplot2::geom_text(ggplot2::aes(x = condition,
                                  y = 0.05,
                                  label = paste0(round(relative*100, 2), "%", "\n",
                                                 "n = ", n)),
                     color = 'white',
                     lineheight = 0.8,
                     position = ggplot2::position_dodge(1)) +
  ggplot2::coord_cartesian(ylim = c(0, 1),
                           expand = 0) +
  ggplot2::scale_fill_manual(values = c('#998ec3', '#f1a340')) +
  ggplot2::geom_errorbar(ggplot2::aes(ymin = relative - ci,
                                      ymax = relative + ci),
                         position = ggplot2::position_dodge(0.9),
                         width = 0.2) +
  papaja::theme_apa() +
  ggplot2::theme(legend.position = c(0.75, 0.9),
        ...
        axis.title.x=ggplot2::element_blank(),
        axis.ticks.x=ggplot2::element_blank())+
  ggplot2::scale_y_continuous("Relative rate of Choosing Focal Lottery (All Treatment)")
```

```{r}
t.test(choose_risky ~ condition, 
       data = study1 %>%
         dplyr::filter(treatment == "treatment") %>%
         dplyr::mutate(condition = dplyr::recode_factor(condition,
                                                        'risk_averse' = 'Safe condition',
